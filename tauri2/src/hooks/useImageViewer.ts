...
import { imageLoad, folderGetImages, openFileDialog, getStartupFile } from '@/lib/api'
...
export function useImageViewer(): ImageViewerState & ImageViewerActions {
  ...
  const loadImage = useCallback(async (filePath: string) => {
    ...
    try {
      ...
      const folder = await folderGetImages(filePath)
  ...
  const navigateImage = useCallback(
    async (index: number) => {
      ...
      try {
        ...
        if (navRequestRef.current === requestId) {
          setError(typeof err === 'string' ? err : (err as Error).message)
        }
      } finally {
  ...
  const reloadCurrent = useCallback(async (): Promise<{ fallbackDir?: string }> => {
    ...
    try {
      // Probe first - imageLoad throws if the file no longer exists
      await imageLoad(image.filePath)
  ...
  }, [image, loadImage, updateFolderImages])

  // Listen for file:open event (second instance / file association)
  useEffect(() => {
  ...
  // Load file passed on first launch (file association / command line)
